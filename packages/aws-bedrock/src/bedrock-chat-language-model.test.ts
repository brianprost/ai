...
import { createBedrock } from './aws-bedrock-provider';

const TEST_PROMPT: LanguageModelV1Prompt = [
  { role: 'user', content: [{ type: 'text', text: 'Hello' }] },
];

const provider = createBedrock({ apiKey: 'test-api-key' });
...
describe('doGenerate', () => {
  ...
  it('should pass custom headers', async () => {
    ...
    await provider.chat('anthropic.claude-3-haiku-20240307-v1:0').doGenerate({
      inputFormat: 'prompt',
      mode: { type: 'regular' },
      prompt: TEST_PROMPT,
    });
  ...
  it('should pass the api key as Authorization header', async () => {
    ...
    const bedrock = createBedrock({ apiKey: 'test-api-key' });

    await bedrock.chat('anthropic.claude-3-haiku-20240307-v1:0').doGenerate({
      inputFormat: 'prompt',
      mode: { type: 'regular' },
      prompt: TEST_PROMPT,
    });
...
describe('doStream', () => {
  ...
  function prepareStreamResponse({ content }: { content: string[] }) {
    server.responseChunks = [
      `data:  {"id":"6e2cd91750904b7092f49bdca9083de1","object":"chat.completion.chunk",` +
        `"created":1711097175,"model":"anthropic.claude-3-haiku-20240307-v1:0","choices":[{"index":0,` +
  ...
  it('should stream tool deltas', async () => {
    ...
    const { stream } = await createBedrock({
      apiKey: 'test-api-key',
      generateId: () => 'test-id',
    })
  ...
  it('should pass custom headers', async () => {
    ...
    await provider.chat('anthropic.claude-3-haiku-20240307-v1:0').doStream({
      inputFormat: 'prompt',
      mode: { type: 'regular' },
      prompt: TEST_PROMPT,
    });
