...
import { z } from 'zod';
import { convertToBedrockChatMessages } from './convert-to-mistral-chat-messages';
import { mapBedrockFinishReason } from './map-mistral-finish-reason';
import {
  BedrockChatModelId,
  BedrockChatSettings,
...
export class BedrockChatLanguageModel implements LanguageModelV1 {
  ...
  async doGenerate(
    ...
    return {
      ...
      finishReason: mapBedrockFinishReason(choice.finish_reason),
      usage: {
  ...
  async doStream(
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<
          ParseResult<z.infer<typeof BedrockChatChunkSchema>>,
          LanguageModelV1StreamPart
        >({
...
// limited version of the schema, focussed on what is needed for the implementation
// this approach limits breakages when the API changes and increases efficiency
const BedrockChatChunkSchema = z.object({
